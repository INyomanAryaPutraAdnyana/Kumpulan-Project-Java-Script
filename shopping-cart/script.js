...
class ShoppingCart {
  ...
  viewProductDetail(id) {
    ...
    document
      ...
      .addEventListener("click", () => shoppingCart.addToCart(product.id));
  ...
  removeFromCart(index) {
    const removedItem = this.cart.splice(index, 1); // tujuan kode ini untuk menghapus dan [0] untuk mendapatkan id yang di apus
    console.log(removedItem);

    this.updateCart();
    this.updateProductQuantity(removedItem.product.id); // untuk update tampilan web menjadi Quantity in cart: 0
  }
...
// button onclick dengan addEventListener

document
  .getElementById("cart-icon")
  .addEventListener("click", shoppingCart.toggleCart);
...
document
  .getElementById("view-details-close-btn")
  .addEventListener("click", shoppingCart.toggleProductDetail);
